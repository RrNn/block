...
class User {
  ...
  async register(data) {
    ...
    const userExists = data.referrer_user_id
      ? await helpers.userExists(data.referrer_user_id)
      : true;

    if (!userExists)
      return `User with refferer id ${data.referrer_user_id} does not exist`;
    ...
    try {
      ...
      if (resp.length > 0) {
        ...
        if (referrer_user_id) {
          await db.query(
            `insert into user_refs (referrer_user_id,referred_user_id) values(${referrer_user_id},${
              resp[0].id
            })`
          );
        }
        ...
        return `Done, an email was sent to ${
          data.email
        }. Click on the link in the email to confirm your email`;
      }
